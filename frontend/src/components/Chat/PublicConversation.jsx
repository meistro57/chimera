            ...
            <div className="max-w-4xl mx-auto space-y-4">
              <div className="bg-white p-4 rounded-lg shadow">
                ...
                <p className="text-sm text-gray-500 mb-4">
                  Shared conversation • Participants: {conversation.participants ? conversation.participants.join(', ') : ''}
                </p>
              </div>

              {conversation.messages && conversation.messages.map((message) => (
                <div key={message.id || Math.random()} className="flex space-x-3">
                  <div className="flex-shrink-0">
                    <div className="w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center text-sm font-medium">
                      {message.sender_id ? message.sender_id[0].toUpperCase() : '?'}
                    </div>
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-1">
                      <span className="font-medium text-gray-900">{message.sender_id || 'Unknown'}</span>
                      {message.persona && (
        ...
        <div className="w-80 border-l border-gray-200 bg-white p-4">
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <p className="text-xs text-blue-700">
              Started: {conversation.created_at ? new Date(conversation.created_at).toLocaleString() : ''}
            </p>
          </div>
